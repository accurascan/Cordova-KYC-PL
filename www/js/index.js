...
function getPermFolder () {
    let path = cordova.file.dataDirectory + "ABCD.jpg";
    ...
    resolveLocalFileSystemURL(
        ...
        err => {
        console.log("We should not be getting an error yet");
        }
    );
}
...
function loadOldImage () {
    ...
    let oldFilePath = localStorage.getItem('ABCD');
    //if there was an old file then load it into the imgFile
    if (oldFilePath) {
      resolveLocalFileSystemURL(
        oldFilePath,
        oldFileEntry => {
          console.log("oldFileEntry.nativeURL:- ", oldFileEntry.nativeURL)
        },
        err => {
...
function startLiveness() {
    ...
    var config = {
        ...
        liveness_url: "https://dev.accurascan.com:4444/upload.php",
    };
    console.log("accuraConfs:- ", accuraConfs)
            ...
            if (result.status) {
                if (result.with_face) {
                    ...
                    if (result.hasOwnProperty('detect')) {
                        $('#detect-face').fadeIn();
                        console.log("result.detect_base64:- ", result.detect_base64)
    ...
    accura.startFaceMatch(accuraConfs, config, function (result) {
        ...
        if (result.hasOwnProperty('status')) {
            if (result.status) {
                ...
                console.log("result.img_1_base64:- ", result.img_1_base64)
                console.log("result.img_2_base64:- ", result.img_2_base64)
